...
impl IRFunctionExpr {
    pub fn function_options(&self) -> FunctionOptions {
        ...
        match self {
            ...
            // Explicit breakpoints are plan-time constants, so every row is decided
            // independently of every other row and this can be pushed into the
            // streaming engine.
            #[cfg(feature = "cutqcut")]
            F::Bin(BinOptions {
            ...
            // The breakpoints are inferred from the whole column, but the resulting
            // value to bin mapping is order-insensitive: row `i` depends only on the
            // value at `i` plus the multiset of all values.
            #[cfg(feature = "cutqcut")]
            F::Bin(BinOptions {
                method:
            ...
            // Rank binning is positional, so ties are split according to the stable
            // sort order and the result *does* observe the input order.
            #[cfg(feature = "cutqcut")]
            F::Bin(BinOptions {
